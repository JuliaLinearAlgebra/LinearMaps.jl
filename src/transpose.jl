...
Base.transpose(A::AbstractLinearMap) = issymmetric(A) ? A : TransposeMap(A)
Base.ctranspose{T<:Real}(A::AbstractLinearMap{T}) = transpose(A)
Base.ctranspose(A::AbstractLinearMap) = ishermitian(A) ? A : CTransposeMap(A)
...
Base.A_mul_B!(y::AbstractVector, A::TransposeMap, x::AbstractVector) = Base.At_mul_B!(y, A.lmap, x)
*(A::TransposeMap,x::AbstractVector) = Base.At_mul_B(A.lmap, x)
...
Base.Ac_mul_B(A::TransposeMap, x::AbstractVector) =
    isreal(A.lmap) ? *(A.lmap,x) : conj!(*(A.lmap,conj(x)))

Base.A_mul_B!(y::AbstractVector, A::CTransposeMap, x::AbstractVector) = Base.Ac_mul_B!(y,A.lmap,x)
*(A::CTransposeMap, x::AbstractVector) = Base.Ac_mul_B(A.lmap,x)
