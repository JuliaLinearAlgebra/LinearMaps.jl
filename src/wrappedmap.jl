immutable WrappedMap{T}<:AbstractLinearMap{T}
    ...
    _ishermitian::Bool
    _isposdef::Bool
    WrappedMap(A::Union{AbstractMatrix{T},AbstractLinearMap{T}};isreal::Bool=Base.isreal(A),issymmetric::Bool=Base.issymmetric(A),ishermitian::Bool=Base.ishermitian(A),isposdef::Bool=Base.isposdef(A))=new(A,isreal,issymmetric,ishermitian,isposdef)
end
WrappedMap{T}(A::Union{AbstractMatrix{T},AbstractLinearMap{T}};isreal::Bool=Base.isreal(A),issymmetric::Bool=Base.issymmetric(A),ishermitian::Bool=Base.ishermitian(A),isposdef::Bool=Base.isposdef(A))=WrappedMap{T}(A;isreal=isreal,issymmetric=issymmetric,ishermitian=ishermitian,isposdef=isposdef)
...
# comparison
==(A::WrappedMap,B::WrappedMap)=(A.lmap==B.lmap && isreal(A)==isreal(B) && issymmetric(A)==issymmetric(B) && ishermitian(A)==ishermitian(B) && isposdef(A)==isposdef(B))
