...
Base.:(+)(A1::LinearMap{T}, A2::UniformScaling) where {T} =
    A1 + UniformScalingMap(convert(promote_type(T, eltype(A2)), A2.λ), size(A1, 1))
Base.:(+)(A1::UniformScaling, A2::LinearMap{T}) where {T} =
    UniformScalingMap(convert(promote_type(eltype(A1), T), A1.λ), size(A2, 1)) + A2
Base.:(-)(A1::LinearMap{T}, A2::UniformScaling) where {T} =
    A1 - UniformScalingMap(convert(promote_type(T, eltype(A2)), A2.λ), size(A1, 1))
Base.:(-)(A1::UniformScaling, A2::LinearMap{T}) where {T} =
    UniformScalingMap(convert(promote_type(T, eltype(A2)), A1.λ), size(A2, 1)) - A2
